...
Инфраструктура запуска тестов (замена pytest).
...
class SkipTest(Exception):
    """Пропуск теста -- тест не применим в текущей конфигурации.
    ...
    Usage:
        def setUp(self):
            if not os.path.exists('./libdiscr.so'):
                raise SkipTest("C-библиотека не скомпилирована")
    """
    pass
...
class TestRunner:
    ...
    def print_summary(self, results: list) -> None:
        ...
        Формат вывода:
            ...
            TEST SUMMARY
            ========================================
            [PASS] TestDiscr.test_cg_basic    (3/3 checks)
            [FAIL] TestDiscr.test_ea_edge     (1/3 checks) ERROR: ...
            [SKIP] TestDiscr.test_sd_large    (библиотека не скомпилирована)
            ----------------------------------------
