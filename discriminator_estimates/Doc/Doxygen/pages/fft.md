...
> **Рекомендация для современного pipeline:** для CPU FFT-задачи
> наилучшим выбором является **`discr5ea` (МНК-Гауссиан по 5 точкам)** с
> fallback на `discr5qa` и обязательным `zero-padding ×2` + окном Hanning.
> Это даёт ошибку ~0.6 Гц при SNR = 20 дБ — в 20 раз лучше argmax и в
> 2 раза лучше классической 3-точечной EA3.
> Полное обоснование, дерево решений и Quick reference — на странице
> @ref recommendations_page.
